    ...
    f.`type` == "application/json")
      new FileReader().futureReadArrayBuffer(f).
        ...
        flatMap(walletNative => importAllKeys(walletNative).
          map(_.pyramidConfig.msg(s"Oh ${pyramidConfig.name()}, we have imported your keys!")))
    else
      Future{pyramidConfig.error(s"Oh ${pyramidConfig.name()}, sorry, we cannot import this data format!")}
