...
import com.eternitas.lastwill.cryptoo.{AsymCrypto, InfuraNative, PinFolder, SymCrypto, WalletNative}
...
class Eternitas(val config: ETConfig) {
  ...
  def exportCredentials()()(implicit ctx: ExecutionContext): Future[js.Dynamic] = Future {
    config.allAuth.
  ...
  def export()(implicit ctx: ExecutionContext) = {
    Future.sequence(
      Seq(exportKeyPair(),
        ...
        exportSignKeyPair(),
        Future {
          config.titleOpt.map(t => l("text" -> t)).getOrElse(l())
        }
      )).
      map(s => l("asym" -> s(0),
        "sym" -> s(1),
